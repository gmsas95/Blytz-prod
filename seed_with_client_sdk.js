// Demo data seeder using Firebase Client SDK
// This script works with your existing Firebase config

const { initializeApp } = require('firebase/app');
const { getFirestore, collection, doc, setDoc, addDoc, serverTimestamp } = require('firebase/firestore');
const { getAuth, createUserWithEmailAndPassword } = require('firebase/auth');

// Use your existing Firebase config from .env
...
// Demo data with realistic structure
const demoData = {
  liveStreams: [
    {
      id: 'stream_001',
...
async function seedDemoData() {
  ...
  try {
    ...
    for (const collection of collections) {
      ...
      for (const item of collection.data) {
        try {
          ...
          console.error(`   ❌ Error with ${collection.name}/${item.id}:`, error.message);
        }
      }
    }
    ...
    console.log('\n💡 Make sure:');
    console.log('   1. Your Firebase config is correct');
...
if (!firebaseConfig.projectId) {
  console.error('❌ Missing Firebase configuration. Please check your .env file for:');
