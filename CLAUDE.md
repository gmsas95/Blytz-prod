...
- **Streaming (AWS IVS):** Use a service abstraction layer (`src/services/streaming.ts`) to manage the SDK, secure URLs, and handle fallbacks.
- **Payments (Fiuu):** Use **Fiuu**. Process 8% platform fee via Cloud Functions. Abstract all logic in `src/services/payments/curlec.ts`.
...
- **Testing:**
    ...
    - **Mocks:** Mock all external APIs (Firebase, AWS IVS, Fiuu, Logistics).
